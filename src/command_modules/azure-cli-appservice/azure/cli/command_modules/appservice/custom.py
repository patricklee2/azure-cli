...
def create_tunnel(cmd, resource_group_name, name, port=None, slot=None):
    ...
    _start_ssh('localhost', tunnel_server.get_port())
...
def _start_ssh(host_name, port):
    subprocess.call("ssh -o StrictHostKeyChecking=no root@{} -p {}".format(host_name, port), shell=True)


def ssh_webapp(cmd, resource_group_name, name, slot=None):  # pylint: disable=too-many-statements
    import platform
    if platform.system() == "Windows":
        logger.warning('webapp ssh is only supported on linux and mac')
    else:
